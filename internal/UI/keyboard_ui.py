    ...
    def __init__(self, master=Tk(),):
        ...
        for key in self.row6keys:
            ...
            if key == "left":
                ...
                self.row6buttons[ind].config(text=key, width=4, bg="#FFFFFF", activebackground=self.gray, command=self.donothing)
            else:
    ...
    def engine(self):
        ...
        for key in self.row6keys:
            ...
            if key == "win":
                ...
                self.row6buttons[ind].config(
                    text="",  # No text for the empty space
                    width=4,  # Keep the same width as the ":)" button
                    bg="#9C9C9C",  # Match the background to the other keys
                    relief=FLAT,  # No border to make it look like an empty space
                    state=DISABLED  # Disable it to prevent interaction
                )
